...
{
    /// <summary>
    /// This class is used to generate the hashes for the internal file paths
    /// </summary>
    public class HashGenerator
    {
        ...
        private static readonly uint[] CrcTable2 = new uint[]{
        ...
        private static readonly uint[] CrcTable3 = new uint[]{
        ...
        private static readonly uint[] CrcTable4 = new uint[]{
        ...
        private static int ComputeCRC(byte[] ecxBytes)
        {
            return ComputeCRC(ecxBytes, 0, ecxBytes.Length);
        }

        private static int ComputeCRC(byte[] ecxBytes, int offset)
        {
        ...
        private static int ComputeCRC(byte[] ecxBytes, int offset, int cbLength)
        {
            using (var stream = new MemoryStream(ecxBytes))
            {
                ...
                {
                    _dwCrc = -1;
                    var cbRunningLength = ((cbLength < 4) ? 0 : ((cbLength) / 4) * 4);
                    var cbEndUnalignedBytes = cbLength - cbRunningLength;
                    for (var i = 0; i < cbRunningLength / 4; ++i)
                    {
                        _dwCrc ^= BitConverter.ToInt32(reader.ReadBytes(4), 0);
                        _dwCrc = unchecked((int)(CrcTable4[_dwCrc & 0x000000FF] ^ CrcTable3[(_dwCrc >> 8) & 0x000000FF] ^ CrcTable2[(_dwCrc >> 16) & 0x000000FF] ^ CrcTable1[(_dwCrc >> 24) & 0x000000FF]));
                    }

                    for (var i = 0; i < cbEndUnalignedBytes; ++i)
                    {
                        var b = reader.ReadByte();
                        if (_dwCrc < 0)
                        {
                            _dwCrc = unchecked((int)(CrcTable1[(_dwCrc ^ b) & 0x000000FF] ^ ((uint)_dwCrc >> 8)));
                        }
                        else
                        {
                            _dwCrc = unchecked((int)(CrcTable1[(_dwCrc ^ b) & 0x000000FF] ^ (_dwCrc >> 8)));
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Gets the hashed value of the given path string
        /// </summary>
        /// <param name="path">The folder path or file name </param>
        /// <returns>The hashed int value of the string</returns>
        public static int GetHash(string path)
        {
            return ComputeCRC(Encoding.Default.GetBytes(path));
        }
    }
}